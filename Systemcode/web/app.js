...
(async () => {
  ...
  let resultFile = "";
  // Set up WebSocket
  const socket = new WebSocket("ws://10.249.145.19:8081");

  socket.onopen = (event) => {
    console.log("WebSocket connected");
    document.getElementsByClassName("status-icon success")[0].classList.add("show");
    document.getElementsByClassName("status-icon error")[0].classList.remove("show");
  };
  ...
  socket.onerror = (event) => {
    ...
    document.getElementsByClassName("status-icon success")[0].classList.remove("show");
  ...
  async function handleOCR() {
    ...
    if (size > limitSize){
        ...
        for(let i=0; i<batches;i++){
            ...
            console.log("send")
            console.log(proceed)
            await sendImage(socket,_image,id,link,proceed)
        }
    }else{
        await sendImage(socket,base64Image,id,link,true)
    }
    // const input = {
    //     "id": ""
...
async function sendImage(socket, image,id,link,proceed){
    const input = {
        "id": id,
        "image": image,
        "link": link,
